  ...
  const uploadMutation = useMutation({
    ...
    onError: (err: unknown) => {
      ...
      showAlert({
        type: 'error',
        title: 'Upload failed',
        message: apiMessage || 'Something went wrong while uploading.',
        duration: 5,
      });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: unknown) => {
      ...
      showAlert({
        type: 'error',
        title: 'Delete failed',
        message: apiMessage || 'Could not remove the document.',
        duration: 4,
      });
    },
  ...
  return (
    <div className="pt-5 poppins-regular space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-[#6B7D6B] mt-1">Upload and manage company documents, policies, and files.</p>
      </div>

      {/* Storage Usage Bar */}
      {storage && (
        <div className="bg-white rounded-2xl border border-[#E4E9E4] p-5 shadow-sm">
          <div className="flex items-center justify-between mb-2.5">
              <div className="flex items-center gap-2 text-[#2C3E2F]">
              <FiHardDrive size={16} />
              <span className="text-sm font-semibold">Storage used</span>
            </div>
              <span className="text-sm text-[#6B7D6B]">
              {storage.usedFormatted} of {storage.limitFormatted}
            </span>
          </div>
            <div className="w-full h-2.5 bg-[#EEF2ED] rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all ${
                  storage.percentUsed >= 90 ? 'bg-rose-500' : storage.percentUsed >= 70 ? 'bg-amber-500' : 'bg-[#8FB978]'
              }`}
      ...
      <div className="bg-white rounded-2xl border border-[#E4E9E4] p-6 shadow-sm">
        <div className="flex items-center gap-2.5 mb-5">
          ...
          <h2 className="text-base font-bold text-[#2C3E2F]">Upload a document</h2>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 sm:grid-cols-2 gap-4" noValidate>
          <div>
            <label className="block text-xs font-semibold text-[#6B7D6B] mb-1.5">Document name</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-xs font-semibold text-[#6B7D6B] mb-1.5">File (PDF, DOC, XLSX)</label>
            <label className="flex items-center gap-2 w-full px-3.5 py-2.5 bg-[#F8FBF5] border border-dashed border-[#DCE3DA] rounded-lg text-sm text-[#6B7D6B] cursor-pointer hover:border-[#8FB978] hover:bg-[#F3F8EE] transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-[#E4E9E4] shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-[#EEF2ED]">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <h2 className="text-base font-bold text-[#2C3E2F]">All documents</h2>

            <div className="relative">
              <FiSearch size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9CAD9B]" />
              <input
                type="text"
                placeholder="Search documents..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="pl-9 pr-3.5 py-2 bg-[#F8FBF5] border border-[#E4E9E4] rounded-lg text-sm text-[#2C3E2F] placeholder-[#9CAD9B] focus:outline-none focus:border-[#8FB978] focus:ring-2 focus:ring-[#DDEAC5] focus:bg-white transition-all w-64"
              />
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="text-left bg-[#F8FBF5] border-b border-[#E4E9E4]">
                  <th className="py-3 px-6 font-semibold text-[#6B7D6B] text-xs uppercase tracking-wide">Name</th>
                  <th className="py-3 px-4 font-semibold text-[#6B7D6B] text-xs uppercase tracking-wide">Uploaded By</th>
              ...
              <tbody>
                {documents.map((doc) => {
                  const iconStyle = fileIconColor[doc.fileType] || 'text-[#6B7D6B] bg-[#EEF2ED]';
                  return (
                    <tr key={doc._id} className="border-b border-[#EEF2ED] last:border-0 hover:bg-[#F3F8EE] transition-colors">
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="font-medium text-[#2C3E2F] truncate">{doc.name}</p>
                            <p className="text-xs text-[#9CAD9B] uppercase">{doc.fileType}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-4 text-[#6B7D6B]">{doc.uploadedBy?.name}</td>
                      <td className="py-4 px-4 text-[#6B7D6B]">{formatDate(doc.createdAt)}</td>
                      <td className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <a
                            ...
                            className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#2C3E2F] border border-[#E4E9E4] hover:bg-[#F3F8EE] px-3 py-1.5 rounded-lg transition-colors"
                          >
                            View
                          </a>
                          <a
                            ...
                            className="inline-flex items-center gap-1.5 text-xs font-semibold text-white bg-[#8FB978] hover:bg-[#7FA867] px-3 py-1.5 rounded-lg transition-colors"
                          >
